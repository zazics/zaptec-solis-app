...
import { 
  ...
  useSolarProductionChartData,
  useGridExchangeChartData,
  useHouseConsumptionChartData,
  useZaptecConsumptionChartData
...
const ChartsScreen: React.FC = () => {
  ...
  const getTabConfig = () => [
    ...
    { key: 'solar', label: 'Production', icon: '☀️' },
    { key: 'house', label: 'Maison', icon: '🏠' },
    { key: 'grid', label: 'Réseau', icon: '🔌' },
        ...
        <TouchableOpacity 
          ...
          onPress={() => {
            Alert.alert(
              ...
              `Impossible de charger les données ${context}. Vérifiez votre connexion.`,
              [{ text: 'OK' }]
            );
          }}
        >
    ...
    switch (activeTab) {
      case 'dashboard':
        ...
        if (dashboardData) {
          return (
            <DashboardChart
              title="Vue d'ensemble énergétique"
              data={dashboardData}
              height={300}
              totalSolarEnergyKwh={dashboardData.totalSolarEnergyKwh}
            />
          );
        }
        return null;
        ...
        if (solarData) {
          return (
            <ChartComponent
              ...
              showValues={true}
              height={280}
              period={solarData.period}
              totalEnergy={solarData.totalEnergyKwh}
            />
          );
        }
        return null;
      ...
      case 'zaptec':
        ...
        if (zaptecData) {
          return (
            <ChartComponent
              title="Consommation Zaptec"
              data={zaptecData.data}
              color="rgba(40, 167, 69, 1)"
              unit="W"
              showValues={true}
              height={280}
              period={zaptecData.period}
            />
          );
        }
        return null;
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          Visualisez l'historique de votre système énergétique
        </Text>
      </View>
      ...
      <PeriodSelector
        ...
        selectedDate={selectedPeriod === 'day' ? selectedDate : undefined}
        onDateChange={selectedPeriod === 'day' ? handleDateChange : undefined}
      />
      ...
      <View style={styles.footer}>
        <Text style={styles.footerText}>
          💡 Utilisez le sélecteur de période et les onglets pour explorer vos données énergétiques
        </Text>
      </View>
    </ScrollView>
  );
